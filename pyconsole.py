...
# DEFAULT_DEVICE_TYPE, given as 'router' then pyconsole will try to enable 
...
class PyConsole:
    """
    Basic class for pyconsole, e.g.
    rtr1 = PyConsole(connection='telnet 10.1.1.1')         ; telnet vty
    rtr2 = PyConsole(connection='telnet 10.1.1.1 2001')    ; telnet console
    rtr3 = PyConsole(connection='ssh -l user 10.1.1.1')    ; ssh 
    """
    ...
    def __init__(self, 
                 ...
                 vendor_os='', version='', debug_level=0):
        ...
        if re.search('pc|linux', self.device_type, re.IGNORECASE): 
            self.CRLF = '\r\n'
        if re.search('ios|cisco', self.vendor_os, re.IGNORECASE): 
            print 'rrrrnn'
            self.CRLF = '\r\n'
    ...
    # ----------------------------------------------------------------------- #
    def dbgmsg(self, msg, msg_level=DEBUG_MSG_VERBOSE):
        '''
    ...
    def expect(self, prompt_list='', timeout=DEFAULT_SHORT_TIMEOUT):
        ...
        try: 
            ...
            self.dbgmsg('Received EOF', 1)

        except pexpect.TIMEOUT:
            self.dbgmsg('Session Timeout', 0)
            pyc_message('expected prompt list', '\n'.join(prompt_list))

        #
        # print detailed debug 
        #
        self.dbgmsg('-----------------------------------', 2)
        self.dbgmsg('prompts : %s' % ' | '.join(prompt_list), 2)
        self.dbgmsg(str(self.child), 9)
    ...
    def sendline_expect(self, cmd, prompt_list='', mode='strip',
        ...
        self.dbgmsg('PyConsole.sendline_expect(): sendline [%s]' % cmd, 
            DEBUG_MSG_VERBOSE)
        self.sendline(cmd)
    ...
    def enable(self):
        ...
        #
        if index == 0: 
            self.dbgmsg('successfully enter enable mode', 9) 
            return 1
        #
    ...
    def connect(self, enable_mode=1):
        ...
        #pyc_message('', 'EXPECT1==>%d' % index)

        #
        # child return output -> 'yes/no', asking confirmation of DSA key
        #
        if index == 0:
            self.dbgmsg('connect_0: be asked for SSH key', 2)

            index, o = self.sendline_expect('yes', 
                ['yes/no', 'sername:', 'assword:', '>', '#', '\$'])

            if index == 0:
                self.dbgmsg('connect_0: error for SSH key', 0)
                return -1
        ...
        #
        if index == 1:
            self.dbgmsg('connect_1: be asked for user', 2)

            index, o = self.sendline_expect(self.user, 
                ['yes/no', 'sername:', 'assword:', '>', '#', '\$'])
            
            if index < 2:
                self.dbgmsg('connect_1: error for user', 0)
                return -1
        ...
        #
        if index == 2:
            self.dbgmsg('PyConsole.connect().2: be asked for passoword',
                DEBUG_MSG_INFO)

            self.dbgmsg('PyConsole.connect().2: sending password [%s]' % 
            ...
            if index < 3:
                self.dbgmsg('connect_2: error for password', 0)
                return -1
        ...
        #
        if index == 3: 
            self.dbgmsg('connect_3: login but not enable mode',
                2)
            ...
            if enable_mode:
                ...
                if self.enable() == -1:
                    self.dbgmsg('connect_3: failed to enter \
                        enable mode', 2)
                    return -1
        ...
        #
        if index == 4: 
            self.dbgmsg('connect_4: login enable mode', 2)
        ...
        #
        if index == 5: 
            self.dbgmsg('connect_5: login linux/pc', 2)

            # prompt '$' means it is a router
            self.device_type = 'pc'

        self.dbgmsg('connect: successufly login router', 1)
    ...
    def page_off(self):
        ...
        # it was pre-set by parameter passed thru. If self.vendor_os=='', we
        # send both commands
        #
        if self.vendor_os == '' or re.search('cisco|ios', self.vendor_os, re.IGNORECASE): 
            self.sendline_expect('terminal len 0')

        if self.vendor_os == '' or re.search('brocade|netiron', self.vendor_os, 
            re.IGNORECASE): 
    ...
    def parse_prompt(self):
        ...
        if a:
            ...
            self.dbgmsg('unexpected hostname and prompt - [%s]' % \
                self.prompt_line, 1)

        self.dbgmsg('pyconsole.parse_prompt->promplist: %s' % 
    ...
    def run_commands(self, cmds, timeout=DEFAULT_SHORT_TIMEOUT):
        ...
        for cmd in cmds.split('\n'):
            self.dbgmsg('\nPyConsole.run_commands(): cmd=[%s]' % cmd, 9)
...
#/ -------------------------- shared functions ------------------------------ \
#/ pyc_pprint(list_name, list_value):                                         |
#| pyc_message(subject, message): pretty print message                        |
#| pyc_ping_pert(output): get ping success rate from router/pc                |
#\ -------------------------------------------------------------------------- /
def pyc_pprint(list_name, list_value, action='print'): 
    str = ''
...
def pyc_message(subject, message):
    '''
...
#                                       ^^ ===> !!!!!!!!!!!!
    ...
    if pert == -1:
        pyc_message('pyc_ping_pert failed to get ping %', output)
...
if __name__ == '__main__':
    ...
    # -------------- test for ssh ------------- #
    _s = PyConsole(connection='ssh -l %s %s' % (usr, ip), \
    ...
    # -------------- test for telnet ------------- #
    _t = PyConsole(connection='telnet %s' % ip,
                   password=pswd)
    ...
    # -------------- test for console ------------- #
    _c = PyConsole(connection='telnet %s' % cs, 
                   password=pswd)
    _c.CRLF = '\r\n'
